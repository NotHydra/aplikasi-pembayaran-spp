...
activity("Mengunjungi halaman pulih tingkat");
...
<body class="hold-transition layout-navbar-fixed layout-fixed" id="body-theme">
                      ...
                      $inputArray = [
                        [
                          ...
                          "value" => [
                            ...
                            }, mysqli_fetch_all(mysqli_query($connection, "SELECT DISTINCT YEAR(dibuat) FROM tingkat WHERE dihapus='1' ORDER BY dibuat DESC;")))), isset($_POST["tahun"]) ? $_POST["tahun"] : null
                          ],
                    ...
                    <div class="col-sm">
                      <table id="main-table" class="table table-bordered table-striped table-sm">
                        ...
                        <tbody>
                          ...
                          $result = mysqli_query($connection, "SELECT id, tingkat, dibuat, diubah FROM tingkat WHERE dihapus='1' $extraFilter ORDER BY dibuat DESC;");
  ...
  if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["id"])) {
      try {
        $id = $_GET["id"];
        $result = mysqli_query($connection, "UPDATE tingkat SET dihapus='0' WHERE id='$id' AND dihapus='1';");

        if ($result) {
          activity("Memulihkan tingkat");
